...
pygame.display.set_model((WINDOW_WIDTH, WINDOW_HEIGHT))
...
score_text, score_rect = (create_text_and_rect
...
game_over_text, game_over_rect = create_text_and_rect(
    text="GAMEOVER",
    color=RED,
...
# TODO: unpack the tuple from create_text_and_rect into a continue_text and continue_rect variabl
#  continue_text, continue_rect = create_text_and_rect(
text="press and space to play again",
color=RED,
background_color=DARKGREEN,
center=(WINDOW_WIDTH // 2, WINDOW_HEIGHT // 2+64) # Slightly below "GAMEOVER"
...
apple_coore = (500, 500, SNAKE_SIZE, SNAKE_SIZE)
apple_rect = pygame.draw.rect(display_surface, RED, apple_coore)
...
def move_snake(event):
    ...
    if event.type == pygame.KEYDOWN:
        ...
        if key == pygame.K_LEFT and snake_dx == 0:
        snake_dx = -SNAKE_SIZE
        snake_dy = 0
...
def check_end_game_after_game_over(event):
    ...
    if event.type == pygame.QUIT:  # Quit the game if the window is closed
        running = False
    elif event.type == pygame.KEYDOWN:  # Detect keypress
        is_paused = False  # Resume the game if any key is pressed
